    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                                ...
                                .then(arg("bind", word())
                                        ...
                                        .then(arg("command", greedyString())
                                                .executes(context -> {
                                                    ...
                                                    if (CrickClient.INSTANCE.macroStorage.getMacro(name) != null) {
                                                    ...
                                                    try {
                                                        ...
                                                        CrickClient.INSTANCE.macroStorage.add(new Macro(
                                                                name,
                                                                command,
                ...
                .then(literal("remove")
                        .then(arg("name", word())
                                .suggests((context, builder1) -> {
                                    CrickClient.INSTANCE.macroStorage.getNames().stream()
                                ...
                                .executes(context -> {
                                    String name = context.getArgument("name", String.class);
                                    if (CrickClient.INSTANCE.macroStorage.isEmpty()) {
                                        ChatUtils.sendMessage("Список макросов пуст!");
                                        return SINGLE_SUCCESS;
                                    }

                                    Macro macro = CrickClient.INSTANCE.macroStorage.getMacro(name);
                                    if (macro == null) {
                                        ChatUtils.sendMessage("Макрос " + name + " не найден!");
                                        return SINGLE_SUCCESS;
                                    }

                                    CrickClient.INSTANCE.macroStorage.remove(macro);
                ...
                .then(literal("list")
                        .executes(context -> {
                            ...
                            if (CrickClient.INSTANCE.macroStorage.getNames().isEmpty()) {
                                ChatUtils.sendMessage("Список макросов пуст!");
                            } else {
                                for (int i = 0; i < CrickClient.INSTANCE.macroStorage.getNames().size(); i++) {
                                    builder1.append(CrickClient.INSTANCE.macroStorage.getNames().get(i));
                                    if (i < CrickClient.INSTANCE.macroStorage.getNames().size() - 1) {
                                        builder1.append(", ");
                                    }
                                }
                                builder1.append(".");
                ...
                .then(literal("clear")
                        .executes(context -> {
                            if (!CrickClient.INSTANCE.macroStorage.isEmpty()) {
                                CrickClient.INSTANCE.macroStorage.clear();
