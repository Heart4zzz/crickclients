    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                .then(literal("save")
                        .then(arg("config", word())
                                .suggests((context, builder1) -> {
                                    if (CrickClient.INSTANCE.configsDir.exists() && CrickClient.INSTANCE.configsDir.isDirectory()) {
                                        File[] files = CrickClient.INSTANCE.configsDir.listFiles((dir, name) -> name.toLowerCase().endsWith(".crick"));

                                        if (files != null) {
                                            Arrays.stream(files)
                                                    .map(File::getName)
                                                    .map(name -> name.replace(".crick", ""))
                                ...
                                .executes(context -> {
                                    ...
                                    try {
                                        CrickClient.INSTANCE.configStorage.saveConfig(config);
                ...
                .then(literal("load")
                        .then(arg("config", word())
                                ...
                                .executes(context -> {
                                    String config = context.getArgument("config", String.class);
                                    try {
                                        CrickClient.INSTANCE.configStorage.loadConfig(config);
                        ...
                        .executes(context -> {
                            ...
                            if (files == null || files.length == 0) {
                                ...
                                for (int i = 0; i < files.length; i++) {
                                    String fileName = files[i].getName().replace(".crick", "");
                ...
                .then(literal("dir")
                        .executes(context -> {
                            try {
                                File configsDir = new File(CrickClient.INSTANCE.globalsDir, "configs");
