    ...
    public void execute(LiteralArgumentBuilder<CommandSource> builder) {
        builder
                .then(literal("add")
                        .then(arg("player", word())
                                ...
                                .executes(context -> {
                                    ...
                                    if (!CrickClient.INSTANCE.staffStorage.isStaff(player)) {
                                        CrickClient.INSTANCE.staffStorage.add(player);
                ...
                .then(literal("remove")
                        .then(arg("player", word())
                                .suggests((context, builder1) -> {
                                    CrickClient.INSTANCE.staffStorage.getStaffs().stream()
                                ...
                                .executes(context -> {
                                    String player = context.getArgument("player", String.class);
                                    if (CrickClient.INSTANCE.staffStorage.isStaff(player)) {
                                        CrickClient.INSTANCE.staffStorage.remove(player);
                ...
                .then(literal("list")
                        .executes(context -> {
                            ...
                            if (CrickClient.INSTANCE.staffStorage.getStaffs().isEmpty()) {
                                ChatUtils.sendMessage("Список стаффов пуст!");
                            } else {
                                for (int i = 0; i < CrickClient.INSTANCE.staffStorage.getStaffs().size(); i++) {
                                    builder1.append(CrickClient.INSTANCE.staffStorage.getStaffs().get(i));
                                    if (i < CrickClient.INSTANCE.staffStorage.getStaffs().size() - 1) {
                                        builder1.append(", ");
                                    }
                                }
                                builder1.append(".");
                ...
                .then(literal("clear")
                        .executes(context -> {
                            if (!CrickClient.INSTANCE.staffStorage.isEmpty()) {
                                CrickClient.INSTANCE.staffStorage.clear();
