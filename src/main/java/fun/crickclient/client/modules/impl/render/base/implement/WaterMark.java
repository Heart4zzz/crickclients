package fun.crickclient.client.modules.impl.render.base.implement;
...
public class WaterMark extends InterfaceProcessing {
    ...
    public void DefaultStyle(EventRender.Default eventRender) {
        ...
        if (!CrickClient.INSTANCE.themeStorage.getThemes().getTheme().getName().equals("Rainbow")) {
            iconTop = CrickClient.INSTANCE.themeStorage.getThemes().getTheme().color[0];
        } else {
        ...
        float crickclientRectX = x;
        float crickclientRectY = y;
        float crickclientRectW = brandTextX + brandTextW + 1.5f - x;

        RenderUtils.drawDefaultHudThemedPanel(matrices, crickclientRectX, crickclientRectY, crickclientRectW, crickclientRectH, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
        }
        ...
        float rect2X = crickclientRectX + crickclientRectW + 2.5f;
        ...
        RenderUtils.drawDefaultHudThemedPanel(matrices, rect2X, crickclientRectY, rect2W, rect2H, 2.8f, 3.3f, iconTop);
        if (drawSquares) {
            RenderUtils.drawHudSquarePattern(matrices, rect2X, crickclientRectY, rect2W, rect2H, iconTop);
        }
        ...
        float rectBtmY = crickclientRectY + crickclientRectH + 2f;
        ...
        float totalW = Math.max(crickclientRectW + 2f + rect2W, rectBtmW);
        draggable.setWidth(totalW);
        draggable.setHeight(showBottom ? (crickclientRectH + 1f + rectBtmH) : crickclientRectH);
    }
    ...
    public void WaveStyle(EventRender.Default eventRender) {
        ...
        String watermarkText = "CrickClient";
    ...
    private float getServerTps() {
        ...
        return Math.max(0.0f, Math.min(20.0f, CrickClient.INSTANCE.tpsCalc.getTPS()));
    }
