...
public class StaffList extends InterfaceProcessing {
    ...
    private void updateStaffCache() {
        ...
        for (Team team : mc.world.getScoreboard().getTeams()) {
            ...
            boolean isInStaffList = CrickClient.INSTANCE.staffStorage.isStaff(name);
        ...
        for (String staffName : CrickClient.INSTANCE.staffStorage.getStaffs()) {
    ...
    private void renderDefaultStyle(EventRender.Default eventRender) {
        ...
        if (!CrickClient.INSTANCE.themeStorage.getThemes().getTheme().getName().equals("Rainbow")) {
            colorTheme = CrickClient.INSTANCE.themeStorage.getThemes().getTheme().color[0];
        } else {
