...
import zov.crickclient.module.list.player.ServerHelper;
...
import zov.crickclient.util.draggable.Draggable;
import zov.crickclient.util.keyboard.KeyStorage;
import zov.crickclient.util.math.Counter;
...
public class Interface extends Module {
    ...
    private static final Identifier CLIENT_ICON = Identifier.of("mre", "images/client_icon.png");
...
//            new BooleanSetting("Таргет худ от темы", false),
    ...
    private final Draggable keyBindsDrag = DragManager.installDrag(this, "HotKeys", 4, 340);
    private final Draggable staffListDrag = DragManager.installDrag(this, "StaffList", 108, 340);
    private final Draggable potionsDrag = DragManager.installDrag(this, "Potions", 212, 340);
    private final Draggable cooldownsDrag = DragManager.installDrag(this, "CoolDowns", 316, 340);
    private final Draggable serverHelperDrag = DragManager.installDrag(this, "ServerHelper", 300, 200);
    private final Draggable targetHUDDrag = DragManager.installDrag(this, "TargetHUD", 420, 310);
    ...
    private void renderKeyBindsNew(DrawContext context) {
        ...
        drawHudHeader("Keybinds", "g", posX, posY, totalRowWidth, headerHeight, headerAlpha);
    ...
    // Клавиша бинда в тёмном бейдже (как на референсе)
    private void drawKeybindCap(String bind, float colX, float rowY, float colW, float itemHeight, int itemAlpha) {
        ...
        DrawUtil.drawRound(capX, capY, capW, capH, 2f, ColorProvider.rgba(255, 255, 255, (int) (14f * itemAlpha / 255f)));
    ...
    private void renderStaffListNew(DrawContext context) {
        ...
        drawHudHeader("Staff List", "i", posX, posY, totalRowWidth, headerHeight, headerAlpha);
    ...
    private final SliderSetting potionsSize = new SliderSetting("Размер бафов", 0.65, 0.5, 2.0, 0.05).setVisible(() -> false);
    ...
    private final BooleanSetting potionsCorners = new BooleanSetting("Уголки бафов", false).setVisible(() -> false);

    // Keybinds
    private final BooleanSetting keybindsBlur = new BooleanSetting("Блюр кейбиндов", true).setVisible(() -> false);
    private final SliderSetting keybindsSize = new SliderSetting("Размер кейбиндов", 0.65, 0.5, 2.0, 0.05).setVisible(() -> false);
    ...
    private final BooleanSetting keybindsCorners = new BooleanSetting("Уголки кейбиндов", false).setVisible(() -> false);

    // StaffList
    private final BooleanSetting stafflistBlur = new BooleanSetting("Блюр стафф", true).setVisible(() -> false);
    private final SliderSetting stafflistSize = new SliderSetting("Размер стафф", 0.65, 0.5, 2.0, 0.05).setVisible(() -> false);
    ...
    private final BooleanSetting stafflistCorners = new BooleanSetting("Уголки списка модераторов", false).setVisible(() -> false);

    // CoolDowns
    private final BooleanSetting cooldownsBlur = new BooleanSetting("Блюр кд", true).setVisible(() -> false);
    private final SliderSetting cooldownsSize = new SliderSetting("Размер кд", 0.65, 0.5, 2.0, 0.05).setVisible(() -> false);
    ...
    private final BooleanSetting cooldownsCorners = new BooleanSetting("Уголки кулдаунов", false).setVisible(() -> false);
    ...
    private final SliderSetting serverHelperSize = new SliderSetting("Размер ServerHelper", 0.65, 0.5, 2.0, 0.05).setVisible(() -> false);
    ...
    private final BooleanSetting serverHelperCorners = new BooleanSetting("Уголки ServerHelper", false).setVisible(() -> false);
    ...
    private final BooleanSetting wmBlur =
            new BooleanSetting("Блюр вм", true).setVisible(() -> false);

    private final SliderSetting wmSize =
            new SliderSetting("Размер вм", 0.65, 0.5, 1.5, 0.05);

    private final SliderSetting wmAlpha =
            new SliderSetting("Прозрачность вм", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting wmShine =
            new BooleanSetting("Блик вм", true).setVisible(() -> false);

    private final SliderSetting wmShineAlpha =
            new SliderSetting("Прозрачность блика вм", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting wmShineThickness =
            new SliderSetting("Толщина блика вм", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting wmCorners =
            new BooleanSetting("Уголки ватермарки", false).setVisible(() -> false);

    private final BooleanSetting wmBps =
            new BooleanSetting("Добавить BPS", false).setVisible(() -> false);

    private final BooleanSetting wmTps =
            new BooleanSetting("Добавить TPS", false).setVisible(() -> false);

    private final BooleanSetting wmCoords =
            new BooleanSetting("Добавить координаты", false).setVisible(() -> false);

    private final HudPopup watermarkPopup = new HudPopup(
            "Watermark",
            watermarkDrag,
            wmBlur,
            wmSize,
...
// ==================== Info ====================
    ...
    private final BooleanSetting infoBlur =
            new BooleanSetting("Блюр инфо", true).setVisible(() -> false);

    private final SliderSetting infoSize =
            new SliderSetting("Размер инфо", 0.70, 0.5, 1.5, 0.05);

    private final SliderSetting infoAlpha =
            new SliderSetting("Прозрачность инфо", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting infoShine =
            new BooleanSetting("Блик инфо", true).setVisible(() -> false);

    private final SliderSetting infoShineAlpha =
            new SliderSetting("Прозрачность блика инфо", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting infoShineThickness =
            new SliderSetting("Толщина блика инфо", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting infoCorners =
            new BooleanSetting("Уголки инфо", false).setVisible(() -> false);

    private final HudPopup infoPopup = new HudPopup(
            "Info",
            infoDrag,
            infoBlur,
            infoSize,
    ...
    {
        infoPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Добавить BPS", wmBps, null)
        );

        infoPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Добавить TPS", wmTps, null)
        );

        infoPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Добавить координаты", wmCoords, null)
        );
    }
...
// ==================== CustomHotbar ====================
    ...
    private final BooleanSetting hotbarBlur =
            new BooleanSetting("Блюр хотбар", true).setVisible(() -> false);

    private final SliderSetting hotbarSize =
            new SliderSetting("Размер хотбар", 0.95, 0.5, 2.0, 0.05).setVisible(() -> false);

    private final SliderSetting hotbarAlpha =
            new SliderSetting("Прозрачность хотбар", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting hotbarShine =
            new BooleanSetting("Блик хотбар", true).setVisible(() -> false);

    private final SliderSetting hotbarShineAlpha =
            new SliderSetting("Прозрачность блика хотбар", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting hotbarShineThickness =
            new SliderSetting("Толщина блика хотбар", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting hotbarCorners =
            new BooleanSetting("Уголки хотбара", true).setVisible(() -> false);

    private final BooleanSetting hotbarCounts =
            new BooleanSetting("Показывать количество", true).setVisible(() -> false);

    private final HudPopup hotbarPopup = new HudPopup(
            "CustomHotbar",
            hotbarDrag,
            hotbarBlur,
            hotbarSize,
            ...
            hotbarShine,
            hotbarShineAlpha,
            hotbarShineThickness,
            hotbarCorners
    );

    {
        hotbarPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Показывать количество", hotbarCounts, null)
        );
    }
...
// ==================== ArmourBar ====================
    ...
    private final BooleanSetting armourBlur =
            new BooleanSetting("Блюр броня", true).setVisible(() -> false);

    private final SliderSetting armourSize =
            new SliderSetting("Размер броня", 0.95, 0.5, 2.0, 0.05).setVisible(() -> false);

    private final SliderSetting armourAlpha =
            new SliderSetting("Прозрачность броня", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting armourShine =
            new BooleanSetting("Блик броня", true).setVisible(() -> false);

    private final SliderSetting armourShineAlpha =
            new SliderSetting("Прозрачность блика броня", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting armourShineThickness =
            new SliderSetting("Толщина блика броня", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting armourCorners =
            new BooleanSetting("Уголки брони", true).setVisible(() -> false);

    private final BooleanSetting armourDurability =
            new BooleanSetting("Полоса прочности", true).setVisible(() -> false);

    private final HudPopup armourPopup = new HudPopup(
            "ArmourBar",
            armourDrag,
            armourBlur,
            armourSize,
            ...
            armourShine,
            armourShineAlpha,
            armourShineThickness,
            armourCorners
    );

    {
        armourPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Полоса прочности", armourDurability, null)
        );
    }
...
// ==================== TotemBar ====================
    ...
    private final BooleanSetting totemBarBlur =
            new BooleanSetting("Блюр тотем-бар", true).setVisible(() -> false);

    private final SliderSetting totemBarSize =
            new SliderSetting("Размер тотем-бар", 0.95, 0.5, 2.0, 0.05).setVisible(() -> false);

    private final SliderSetting totemBarAlpha =
            new SliderSetting("Прозрачность тотем-бар", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting totemBarShine =
            new BooleanSetting("Блик тотем-бар", true).setVisible(() -> false);

    private final SliderSetting totemBarShineAlpha =
            new SliderSetting("Прозрачность блика тотем-бар", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting totemBarShineThickness =
            new SliderSetting("Толщина блика тотем-бар", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting totemBarCorners =
            new BooleanSetting("Уголки полосы тотемов", true).setVisible(() -> false);

    private final HudPopup totemBarPopup = new HudPopup(
            "TotemBar",
            totemBarDrag,
            totemBarBlur,
            totemBarSize,
...
// ==================== TargetHUD ====================
    ...
    private final BooleanSetting thBlur =
            new BooleanSetting("Блюр тх", true).setVisible(() -> false);

    private final SliderSetting thSize =
            new SliderSetting("Размер тх", 0.85, 0.5, 2.0, 0.05).setVisible(() -> false);

    private final SliderSetting thAlpha =
            new SliderSetting("Прозрачность тх", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting thShine =
            new BooleanSetting("Блик тх", true).setVisible(() -> false);

    private final SliderSetting thShineAlpha =
            new SliderSetting("Прозрачность блика тх", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting thShineThickness =
            new SliderSetting("Толщина блика тх", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting thCorners =
            new BooleanSetting("Уголки таргет худа", false).setVisible(() -> false);

    private final BooleanSetting thShowOnHover =
            new BooleanSetting("Показывать при наведении", true).setVisible(() -> false);

    private final BooleanSetting thShowItems =
            new BooleanSetting("Показывать предметы", true).setVisible(() -> false);

    private final HudPopup targetHudPopup = new HudPopup(
            "TargetHUD",
            targetHUDDrag,
            thBlur,
            thSize,
    ...
    {
        targetHudPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Показывать при наведении", thShowOnHover, null)
        );

        targetHudPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Показывать предметы", thShowItems, null)
        );
    }
...
// ==================== Notifications ====================
    ...
    private final Draggable notificationsDrag =
            DragManager.installDrag(this, "Notifications", 250, 250);

    private final BooleanSetting ntBlur =
            new BooleanSetting("Блюр notif", true).setVisible(() -> false);

    private final SliderSetting ntSize =
            new SliderSetting("Размер notif", 0.80, 0.5, 2.0, 0.05).setVisible(() -> false);

    private final SliderSetting ntAlpha =
            new SliderSetting("Прозрачность notif", 180, 0, 255, 1).setVisible(() -> false);

    private final BooleanSetting ntShine =
            new BooleanSetting("Блик notif", true).setVisible(() -> false);

    private final SliderSetting ntShineAlpha =
            new SliderSetting("Прозрачность блика notif", 30, 0, 100, 1).setVisible(() -> false);

    private final SliderSetting ntShineThickness =
            new SliderSetting("Толщина блика notif", 0.5, 0.2, 2.0, 0.05).setVisible(() -> false);

    private final BooleanSetting ntCorners =
            new BooleanSetting("Уголки уведомлений", true).setVisible(() -> false);

    private final BooleanSetting notifModuleStates =
            new BooleanSetting("Состояния модулей", true).setVisible(() -> false);

    private final BooleanSetting notifTotem =
            new BooleanSetting("Снос тотема", true).setVisible(() -> false);

    private final HudPopup notificationsPopup = new HudPopup(
            "Notifications",
            notificationsDrag,
            ntBlur,
            ntSize,
    ...
    {
        notificationsPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Состояния модулей", notifModuleStates, null)
        );

        notificationsPopup.extraRows.add(
                new PopupRow(PopupKind.TOGGLE, "Снос тотема", notifTotem, null)
        );
    }
    ...
    private void beginScale(HudPopup p, DrawContext context) {
        ...
        if (needScale) {
            zov.crickclient.util.render.renderers.IRenderer.DEFAULT_MATRIX
    ...
    private void endScale(HudPopup p, DrawContext context) {
        ...
        context.getMatrices().pop();
        zov.crickclient.util.render.renderers.IRenderer.DEFAULT_MATRIX.identity();
    }
    ...
    // Единый фон элемента: чёрное стекло + blur, без цветных уголков темы
    private void drawElementBackground(HudPopup p, float x, float y, float w, float h, float radius, float alphaFactor) {
        ...
        int bg = ColorProvider.rgba(0, 0, 0, alpha);
        if (p.blur.getValue()) {
            ...
            DrawUtil.drawRound(x, y, w, h, radius, bg);
        } else {
            DrawUtil.drawRound(x, y, w, h, radius, bg);
        }
    }
    ...
    private void drawHudHeader(String title, String iconGlyph, float x, float y, float w, float headerH, int alpha) {
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), title, x + 5f, y + 2.5f, ColorProvider.rgba(255, 255, 255, alpha), 8f);
        float iconW = Fonts.CRICKCLIENT.get().getWidth(iconGlyph, 9f);
        DrawUtil.drawText(Fonts.CRICKCLIENT.get(), iconGlyph, x + w - 5f - iconW, y + 3f,
                ColorProvider.setAlpha(ColorProvider.getColorIcons(), alpha), 9f);
        DrawUtil.drawRound(x + 4f, y + headerH - 0.5f, w - 8f, 0.5f, 0.25f,
                ColorProvider.rgba(255, 255, 255, Math.min(255, Math.max(0, alpha / 10))));
    }
    ...
    private void drawElementShine(HudPopup p, DrawContext context, float x, float y, float w, float h, float radius) {
        ...
        zov.crickclient.util.render.renderers.impl.HudShine.render(
    ...
    private void renderPotionsNew(DrawContext context) {
        ...
        for (PotionItem item : potionItems) {
            ...
            if (animVal > 0.001f) {
                ...
                float nameW = Fonts.SFMEDIUM.get().getWidth(item.name + lvlStr, 7f);
        ...
        drawHudHeader("Effects", "f", posX, posY, totalRowWidth, headerHeight, headerAlpha);
        ...
        for (PotionItem item : potionItems) {
            ...
            zov.crickclient.util.render.timer.TimerTextAnimator.draw(
                Fonts.SFMEDIUM.get(),
                timerKey,
                timeStr,
    ...
    private void renderWatermarkNew(DrawContext context) {
        ...
        int whiteColor = -1;
        int sepColor = ColorProvider.rgba(255, 255, 255, 55);

        String title = "Crick Client";
        float iconSize = 9f;
        float titleW = Fonts.SFMEDIUM.get().getWidth(title, fontSize);
        float userW = Fonts.SFMEDIUM.get().getWidth(userText, fontSize);
        float fpsW = Fonts.SFMEDIUM.get().getWidth(fpsValue, fontSize);
        float pingW = Fonts.SFMEDIUM.get().getWidth(pingValue, fontSize);
        float sepW = Fonts.SFMEDIUM.get().getWidth("|", fontSize);
        ...
        float boxWidth = leftPad + iconSize + iconGap + titleW
        ...
        cx += iconSize + iconGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), title, cx, textY, whiteColor, fontSize);
        cx += titleW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), "|", cx, textY, sepColor, fontSize);
        cx += sepW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), userText, cx, textY, whiteColor, fontSize);
        cx += userW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), "|", cx, textY, sepColor, fontSize);
        cx += sepW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), fpsValue, cx, textY, whiteColor, fontSize);
        cx += fpsW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), "|", cx, textY, sepColor, fontSize);
        cx += sepW + sepGap;
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), pingValue, cx, textY, whiteColor, fontSize);
    ...
    private void drawWatermarkClientIcon(DrawContext context, float x, float y, float size, int tint) {
        try {
            RenderSystem.setShaderTexture(0, CLIENT_ICON);
            int texId = mc.getTextureManager().getTexture(CLIENT_ICON).getGlId();
            if (texId > 0) {
                Builder.texture()
        ...
        DrawUtil.drawText(Fonts.ICONS2.get(), "\u0034", x, y + 1f, tint, 7f);
    }
    ...
    private static String formatHudDuration(float seconds) {
        ...
        return String.format(java.util.Locale.US, "%.2f сек", seconds);
    }
    ...
    private void drawCircularHealthRing(float cx, float cy, float radius, float thickness,
        ...
        DrawUtil.drawRingArc(cx, cy, radius, thickness, -90f, 270f, ColorProvider.setAlpha(trackColor, alphaInt));
        if (progress > 0.001f) {
            DrawUtil.drawRingArc(cx, cy, radius, thickness, -90f, -90f + 360f * progress,
                    ColorProvider.setAlpha(fillColor, alphaInt));
        }
    }
    ...
    private void renderInfoNew(DrawContext context) {
        ...
        String tpsValue = String.format(java.util.Locale.US, "%.1f Tps", CrickClient.getInstance().getTpsGetter().getTPS());
    ...
    private void renderTargetHUDContent(DrawContext context) {
        ...
        if (killAura.isEnabled() && killAura.getTarget() != null && killAura.getTarget().isAlive()) {
            ...
            if (!(living instanceof net.minecraft.entity.player.PlayerEntity p && zov.crickclient.module.list.combat.AntiBot.isBot(p))) {
                target = living;
            }
        }
        else if (chatOpen) {
        ...
        try {
            ...
            if (faceTex != null) {
                ...
                if (texId > 0) {
                    zov.crickclient.util.render.renderers.impl.BuiltTexture headTexture = Builder.texture()
                            .size(new SizeState(headSize, headSize))
        ...
        zov.crickclient.module.list.misc.NameProtect nameProtect = Instance.get(zov.crickclient.module.list.misc.NameProtect.class);
        ...
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), shownName, textX, y + 8f, ColorProvider.rgba(255, 255, 255, alphaInt), 8.5f);

        float currentHp = zov.crickclient.util.server.Server.getHealth(livingEntity, false);
        ...
        int trackColor = ColorProvider.rgba(255, 255, 255, 28);
        int fillColor = currentHp <= lowHpAlertThreshold.getFloatValue()
                ? ColorProvider.rgba(255, 80, 80, 255)
        ...
        String hpText = zov.crickclient.util.server.Server.formatHealthDisplay(currentHp);
        float hpTextW = Fonts.SFMEDIUM.get().getWidth(hpText, 7.5f);
        DrawUtil.drawText(Fonts.SFMEDIUM.get(), hpText, ringCX - hpTextW / 2f, ringCY - 5f,
    ...
    private void renderCoolDownsNew(DrawContext context) {
        ...
        for (CooldownItem item : cooldownItems) {
            ...
            if (animVal > 0.001f) {
                ...
                String timeStr = formatHudDuration(sec);
                float tw = Fonts.SFMEDIUM.get().getWidth(timeStr, 6.75f) + 10f
        ...
        drawHudHeader("Cooldowns", "h", posX, posY, totalRowWidth, headerHeight, headerAlpha);
    ...
    private void renderServerHelperNew(DrawContext context) {
        ...
        ServerHelper serverHelper = CrickClient.getInstance().getModuleStorage().get(ServerHelper.class);
        ...
        for (ServerHelperItem item : items) {
            ...
            if (serverHelperPopup.blur.getValue()) {
                ...
                DrawUtil.drawRound(curX, posY, boxSize, boxSize, radius, ColorProvider.rgba(0, 0, 0, alpha));
            } else {
                DrawUtil.drawRound(curX, posY, boxSize, boxSize, radius, ColorProvider.rgba(0, 0, 0, alpha));
            }
