package rw.core;

import KDFzREm.NNNEL;
import KDFzREm.NNNZg;
import KDFzREm.NNNvM;
import KDFzREm.NNNvY;
import KDFzREm.NNrm;
import KDFzREm.NNtJ;
import KDFzREm.Ncv;

public class C0226 implements NNNvY<Ncv> {
   private final NNtJ f1000;

   public C0226(NNtJ var1) {
      this.f1000 = var1;
   }

   public NNNvM m2000(Ncv var1, NNNZg var2, double var3, double var5, double var7, double var9, double var11, double var13, NNrm var15) {
      return new NNNEL(var2, var3, var5, var7, var9, var11, var13, this.f1000.method_18139(var15));
   }
}
